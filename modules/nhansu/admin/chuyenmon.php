...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' . $db_config['prefix'] . '_' . $module_data . '_chuyenmon WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_chuyenmon SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_chuyenmon  WHERE id = ' . $db->quote($id));
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete chuyenmon', 'ID: ' . $id, $admin_info['userid']);
...
if ( !empty( $q ) ) {
    $where .= ' AND (name_chuyenmon LIKE "%" "'.$q. '" "%")';
    $base_url .= '&q=' . $q;
}
...
$db->sqlreset()
->select('COUNT(*)')
->from('' . $db_config['prefix'] . '_' . $module_data . '_chuyenmon')
->where('1=1'.$where);
...
$xtpl = new XTemplate('chuyenmon.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$xtpl->assign('OP', $op);
$xtpl->assign('Q', $q);
$xtpl->assign( 'chuyenmon_add', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=chuyenmon_add');
if ( $ngay_tu > 0 )
...
while ($view = $sth->fetch()) {
    ...
    $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=chuyenmon_add&amp;id=' . $view['id'];
...
$page_title = $lang_module['chuyenmon'];
